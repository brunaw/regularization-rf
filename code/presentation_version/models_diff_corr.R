...
da <- readRDS("code/presentation_version/data/da100corr.rds")
...
# Applying models
system.time(
  models_corr <- da %>% map(run_models)
)
# saveRDS(models_corr, "code/presentation_version/data/models_corr.rds")
models_corr <- readRDS("code/presentation_version/data/models_corr.rds")
...
p2 <- res_pars_formatted %>% 
  ...
  scale_fill_manual(
    ...
    labels = c(
      ...
      "Correlation as regularization parameter",
...
write.table(ind, 
            file = 
              "code/presentation_version/data/ind_vars_models_corr.txt")
